...
function setupEventListeners() {
    ...
    // Real-time parsing toggle
    const realtimeToggle = document.getElementById('realtimeToggle');
    if (realtimeToggle) {
        realtimeToggle.addEventListener('change', function() {
            toggleRealtimeParsing(this.checked);
        });
    }
...
function toggleRealtimeParsing(enabled) {
    window.realtimeEnabled = enabled;
    if (enabled) {
        showInfo('⚡ Real-time parsing enabled');
        parseRMLContent(); // Parse current content
    } else {
        showInfo('⏸️ Real-time parsing disabled');
    }
}
